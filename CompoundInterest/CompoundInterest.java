...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
                              int targetYear) {
        double amount =0 ;
        int NumberofYears= numYears(targetYear);
        
        while (NumberofYears > 0){
            amount = presentValue*((rate/100) + 1);
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        double amount=0;
        int NumberofYears = numYears(targetYear);
            amount = futureValue(presentValue, rate, targetYear)*Math.pow(((-inflationRate/100)+1), NumberofYears);
            return amount;
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        int NumberofYears = numYears(targetYear);
            return perYear*((rate/100)+1)*NumberofYears;
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        return perYear*((inflationRate/100)+1)*numYears(targetYear);
